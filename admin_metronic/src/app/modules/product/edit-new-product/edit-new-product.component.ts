...
export class EditNewProductComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._productService.showProduct(this.product_id).subscribe((resp: any) => {
      ...
      this.price_euros = this.product_selected.price_euros;
  ...
  processFileGaleria($event) {
    ...
    reader.onloadend = () => this.imagen_previs_galeria = reader.result;//resultado en basex64
    this.loadServices();
  }
  ...
  update() {
    ...
    this._productService.updateProduct(formData).subscribe((resp: any) => {
      ...
      if (resp.code == 403) {
        ...
        this.toaster.open(NoticyAlertComponent, { text: `primary-'Eyy! El producto se edito con exito.'` });
        return;
      }
    });
  }
  ...
  saveVariedad() {
    ...
    if (!this.valor_multiple
      || !this.stock_multiple) {
      this.toaster.open(NoticyAlertComponent, { text: `danger-'Eyy! Es necesario insertar un valor y un stock.'` });
      return;
    }
    let data = {
