...
export class EditNewCuponeComponent implements OnInit {
  ...
  showCupon() {
    this._cuponService.showCupon(this.cupone_id).subscribe((resp: any) => {
      ...
      // this.cupone_id = this.cupone_selected.cupone_id;
  ...
  addProductOrCategorie() {
    if (this.type_segment == 1) {
      ...
      if (INDEX != -1) {
        this.toaster.open(NoticyAlertComponent, { text: `danger-'Upps! El producto ya existe, seleciona otro.'` });
        return;
      } else {
      ...
      if (INDEX != -1) {
        this.toaster.open(NoticyAlertComponent, { text: `danger-'Upps! La categoria ya existe, seleciona otro.'` });
        return;
      } else {
  ...
  update() {
    ...
    if (this.type_count == 2) {
      if (this.num_use == 0) {
        this.toaster.open(NoticyAlertComponent, { text: `danger-'Upps! Tienes que rellenar el numero de usos.'` });
        return;
      }
    }
    if (this.type_segment == 1) {
